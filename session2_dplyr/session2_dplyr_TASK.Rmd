...
When working with datasets, they won't often come in the form that you want or need them - in this case it may be necessary to group variables, create subsets of data, arrange observations or add new variables entirely to make things easier to understand. Most of this **can be achieved** using base R commands but `dplyr` provides an alternative (neater, simpler?) way to do these manipulations. The `dplyr` package provides a 'grammar' of data manipulation and comes with 5 core functions which can be used to solve most data manupulation challenges: 
...
The tasks below can be completed using the dplyr verbs as your core functions, however (as you may have seen) in R/coding there are often multiple solutions to the same problem, so feel free to explore these and use your existing knowledge to answer the below - you can then compare your approach with the dplyr method and see which you prefer. 
...
measles_2019 <-
  measles_data %>% select(
    ...
    member_state,
    country_code,
    confirmed_cases_2019,
    incidence_rate_per_1mil_2019
  )

#OR

# measles_data %>% select(Region:country_code, confirmed_cases_2019:incidence_rate_per_1mil_2019)

#OR

# measles_data %>% select(-(suspected_cases_2020:confirmed_cases_2020))
...
Creating a new dataframe for the measles data
...
### Part BB: Using `filter()`

1. Create a dataframe which contains only measles data for the European region (EURO)

With dplyr:

```{r echo = TRUE}

# Create a dataset which contains only measles data for the EURO region
measles_EURO <- measles_data %>% filter(Region == "EURO")

```

With baseR:

- `measles_EURO <- subset(measles_data, Region == "EURO")` - using the subset function (similar structure to `filter()`)

OR

- `measles_EURO <- measles_data[which(measles_data$Region == "EURO"), ]`
